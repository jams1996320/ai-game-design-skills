...
macOS/Linux（已有 Python 3.9–3.12，推荐 3.12）：

```sh
python3 "<scripts>/setup.py"
```

可用 `--root <目录>` 指定运行环境位置。安装包来自 PyPI；固定版本便于测试，不代表完整供应链哈希锁定。当前目标为 Windows x64，安装脚本拒绝 ARM64/32 位系统并选择 64 位 Python 3.9–3.12（缺失时安装 3.12 x64）；Windows 脚本待实机验证。没有 winget/Python、下载失败、受管权限拒绝时，报告准确原因，不反复盲试。
